...
void ImGuiConsole::DrawIfVisible(const char* title, ConsoleMode consoleMode)
{
	...
	if (ImGui::BeginMenuBar())
	{
		if (showMenuBar)
		{
			...
			{
				ImGui::SliderFloat("半透明时的透明度", &window_bgAlphaWhenSemi, 0.1f, 1.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp);
			...
			ImGui::Separator();
			ImGui::Checkbox("刷屏测试", &doSpamInConsole);
			ImGui::Separator();
		}
	...
	{
		if (ImGui::Selectable("复制当前行"))
		{
			ImGui::SetClipboardText(m_SingleLineToCopy.c_str());
		}
		if (ImGui::Selectable("全部复制")) copy_to_clipboard = true;
