...
void ParkourLog::DrawDisplayControls()
{
    ...
    ImGui::Checkbox("早期丢弃（有时会影响性能）", &m_DisplaySettings.m_ShowDiscardedEarly);
    ImGui::Checkbox("晚期丢弃", &m_DisplaySettings.m_ShowDiscardedLate);
    ImGui::Checkbox("未丢弃", &m_DisplaySettings.m_ShowNondiscarded);
    ImGui::NewLine();
    ImGui::Checkbox("显示多个最近周期（有时会影响性能）", &m_DisplaySettings.m_ShowMoreThanOneOfTheRecentCycles);
    {
        ImGuiCTX::Indent _ind;
        ImGui::Checkbox("跳过完全丢弃的周期", &m_DisplaySettings.m_ShowMoreThanOneOfTheRecentCycles_SkipAllDiscarded);
    }
