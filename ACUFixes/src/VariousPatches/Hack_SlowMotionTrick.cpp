...
{
    ImGui::Checkbox("启用慢动作特效", &g_Config.hacks->slowmotionTrick->isActive.get());
    if (ImGui::IsItemHovered())
    {
        ImGui::SetTooltip(
            "按下热键（默认「鼠标按键4」）激活一个短暂的慢动作序列。\n"
            "试试在某个酷炫动作_刚完成后_立即按下。"
        );
    }
    if (g_Config.hacks->slowmotionTrick->isActive)
    {
        ...
        ImGui::DrawEnumPicker("慢动作特效热键", g_Config.hacks->slowmotionTrick->hotkey.get(), ImGuiComboFlags_HeightLarge);
    }
}
