...
void DrawBatlampControls()
{
    ...
    ImGui::Button("将鼠标悬停在此处查看说明");
    ImGui::PopStyleColor(3);
    if (ImGui::IsItemHovered())
    {
        ImGui::SetTooltip(
            "你可以在《死去的国王》DLC 最后一关之外使用圣但尼之首。\n"
            "你可以选择：\n"
            "- 每次重新加载关卡时，自动用魔法提灯替换普通提灯；\n"
            "- 尝试手动添加魔法提灯。\n"
            "可使用新热键（默认为 \"N\"）进入和退出「可充能」模式。\n"
            "如果你同时启用了「假装在法兰西亚德」作弊，甚至可以在巴黎使用提灯。\n"
            "在巴黎使用提灯的副作用至少会有一些音效缺失。\n"
            "请务必在使用此功能前备份存档。"
        );
    }
    ...
    {
        ImGui::SetTooltip(
            "下次重新加载关卡时，普通提灯将被圣但尼之首替换，"
            "\n如同在《死去的国王》DLC 最终关卡中使用的那样。"
        );
    }
    ImGui::Checkbox("使用魔法提灯时解锁移动",
    ...
    if (lanternCpnt)
    {
        ImGui::Checkbox("允许现在充能", (bool*)&lanternCpnt->isInModeAbleToCharge_300);
        ImGui::DrawEnumPicker("可充能模式切换键",
            g_Config.cheats->batlampOfFranciadeManipulations->batlampChargeModeButton.get(),
            ImGuiComboFlags_HeightLarge);
    }
    ImGui::BulletText("找不到 `LanterndlcComponent`");
    if (ImGui::Button("尝试手动添加魔法提灯"))
    {
