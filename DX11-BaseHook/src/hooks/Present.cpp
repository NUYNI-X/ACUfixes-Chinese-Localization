...
fs::path FindPreferredFontFile()
{
    ...
    // Microsoft YaHei supports Chinese (CJK) characters and is available on all Windows 10/11 systems.
    fs::path preferredFont =        fontsDir / "msyh.ttc";  // Microsoft YaHei
    fs::path secondPreferredFont =  fontsDir / "simsun.ttc"; // SimSun
    fs::path thirdPreferredFont =   fontsDir / "CascadiaCode.ttf";
    std::error_code ec;
...
void SetupFonts()
{
    ...
    // Build custom glyph ranges: Chinese Full + Cyrillic (Russian)
    ...
    rangesBuilder.BuildRanges(&combinedRanges);
